...
{/////////////////////////////////////////////////////////////////////////////////////////////////////////>
  ...
  var                       _iDAYSdATA_sDate                    =  0;                                   //> Table column indexes. sDate         ,1999-12-31T12:00:00  First cell is nYear as text in ISO format yyyy-mm-dd. 
  var                       _iDAYSdATA_nDay_jd                  =  1;                                   //>                       a_nDay_jd     ,2451544              Numeric version of key.
  var                       _iDAYSdATA_dSunSet_h                =  2;                                   //>                       dSunSet_h     ,16.026944444444442 
  var                       _iDAYSdATA_dSunRise_h               =  3;                                   //>                       dSunRise_h    ,32.06888888888889 
  var                       _iDAYSdATA_dMoonRise_h              =  4;                                   //>                       r_dMoonRise_h ,26.668333333333333 
  var                       _iDAYSdATA_dMoonSet_h               =  5;                                   //>                       dMoonSet_h    ,37.220555555555556 
  var                       _iDAYSdATA_Moon_sDate               =  6;                                   //>                       Moon_sDate    ,2000-01-01T13:13:14 
  var                       _iDAYSdATA_Moon_dLit_100            =  7;                                   //>                       dLit_100      ,22.799720764160156 
  var                       _iDAYSdATA_Moon_dRa                 =  8;                                   //>                       dRa           ,-137.51418852173234 
  var                       _iDAYSdATA_Moon_dDec                =  9;                                   //>                       dDec          ,-11.811177408287486 
  var                       _iDAYSdATA_MidN_sDate               = 10;                                   //>                       MidN_sDate    ,2000-01-01T00:00:00 
  var                       _iDAYSdATA_MidN_dLit_100            = 11;                                   //>                       dLit_100      ,26.628517150878906 
  var                       _iDAYSdATA_MidN_dRa                 = 12;                                   //>                       dRa           ,-142.82009664343948 
  var                       _iDAYSdATA_MidN_dDec                = 13;                                   //>                       dDec          ,-9.647356542776926 
  var                       _iDAYSdATA_Vega_sDate               = 14;                                   //>                       Vega_sDate    ,1999-12-31T23:59:50 
  var                       _iDAYSdATA_Vega_dLit_100            = 15;                                   //>                       dLit_100      ,26.629600524902344 
  var                       _iDAYSdATA_Vega_dRa                 = 16;                                   //>                       dRa           ,-142.82155355617718 
  var                       _iDAYSdATA_Vega_dDec                = 17;                                   //>                       dDec          ,-9.646872946743024 
...
//.  }//for d                                                                                              //>
 ...
 ,                      a_sWhenLook                                                                     //> "Moon", "MidN", or "Vega".
  ...
  if( null == asDaysData ){                                                                 return "*";}//> Look up data for the given day. If there is a problem, like date isn't in table, then quit.
                                                                                                        // trace("sWinterObservations "+ a_sGo +" "+ n_jd +" "+ asDaysData);//>
//. [2000-02-15T12:00:00       // _iDAYSdATA_sDate         =  0;         
//. ,2451590                   // _iDAYSdATA_nDay_jd       =  1;         
//. ,17.242222222222225        // _iDAYSdATA_dSunSet_h     =  2;  Hours (UTC, since last midnight)
//. ,31.211111111111112        // _iDAYSdATA_dSunRise_h    =  3;  24+ because this is the number of hours from UTC start of day, to end of one continuous night.
//. ,12.511944444444444        // _iDAYSdATA_dMoonRise_h   =  4;  Rises just after noon in this case      

//. ,28.874722222222225        // _iDAYSdATA_dMoonSet_h    =  5;  24+ because this is the number of hours from UTC start of day, to moonset during the night.
//. ,2000-02-16T04:52:29       // _iDAYSdATA_Moon_sDate    =  6;  Moonset time       
//. ,83.7523193359375          // _iDAYSdATA_Moon_dLit_100 =  7;  83% - we missed 1st quarter moon.       
//. ,99.84598216989706         // _iDAYSdATA_Moon_dRa      =  8;         
//. ,20.129735102040208        // _iDAYSdATA_Moon_dDec     =  9;         

//. ,2000-02-16T00:00:00       // _iDAYSdATA_MidN_sDate    = 10; Earlier at midnight...
//. ,82.00177001953125         // _iDAYSdATA_MidN_dLit_100 = 11; 1% less illumination                
//. ,96.78695239100111         // _iDAYSdATA_MidN_dRa      = 12; 3 degrees movement 5 hours?        
//. ,20.314673377009978        // _iDAYSdATA_MidN_dDec     = 13;         

//. ,2000-02-15T20:58:59       // _iDAYSdATA_Vega_sDate    = 14;         
//. ,81.1370849609375          // _iDAYSdATA_Vega_dLit_100 = 15;         
//. ,95.28642241319756         // _iDAYSdATA_Vega_dRa      = 16; 5 degrees movement in 8 hours?        
//. ,20.313803702885604        // _iDAYSdATA_Vega_dDec     = 17;         
...
//,2000-03-14T02:44:37
  ...
  n_jd += 7;                                                                                            //> we skip ahead to the big night.
  asDaysData    = _mapasDaysData[ n_jd +"jd" ];                                                         //> Look up data for the given date.
  if( null == asDaysData ){                                         trace("???");           return "*";}//>
                                                                                                        //>
  var                                   iDate                   = 0;                                    //> Indexes
  ...
  if( d_360 < 0 ){   n_jd += 337;   r_s += ", 337";   }                                                 //> If it has, then skip ahead to next year.
                                         ...
                                         // , 366                                                       //>
 return r_s +","+ sYmd( n_jd );          // ,2001-03-22                                                 //> Report results, with row ending in next year's date.
 }//sWinterObservations///////////////////////////////////////////////////////////////////////////////////>
...
// if( 0 < TEST_nYearFromJd() ){                                                                return;}//>
  ...
  for( y in 2000...2100 ){                                                                              //> For a century...
   Go_ReadMapasDataFromFile( '2000_2099/table012_'+ y +'.csv' );                                        //> Get data from one file, put in global table
                                                                                                        ...
                                                                                                        //> RUNNING TRIALS:
  for( y in 2000...2020 ){                                                                              //> For each year in a test run (starting with different years just to mix things up for testing)...
   ...
   //trace("Year = "+ y +" "+ s1 +"-------------------------------");                                      //>
   for( z in 0...20 ){                                                                                   //> Run observations for 20 years in a row  (hoping they converge on a winter date)...
    s1 = sWinterObservations( s1 ,"Moon" );                                                              //> Run the "Hey Diddle Diddle algorithm" from one starting date. Add a row to the results.
    //trace(s1);
    sReport += s1 + "\n";                                                                                //>
